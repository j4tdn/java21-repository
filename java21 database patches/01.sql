...
CREATE TABLE T01_ITEM (
    ...
    -- vì 1 table chỉ có duy nhất 1 khóa chính và luôn lấy luôn name = PRIMARY KEY
    ...
    CONSTRAINT FK_ITEM_ITEM_GROUP FOREIGN KEY(C01_ITEM_GROUP_ID) REFERENCES T02_ITEM_GROUP(C02_ITEM_GROUP_ID),
	CONSTRAINT CHECK_BUY_PRICE_POSITIVE CHECK(C01_BUY_PRICE >= 0)
    -- mặc định là ON DELETE(NO ACTION = RESTRICT), ON UPDATE(NO ACTION = RESTRICT)
    -- khi xóa dòng dữ liệu ở 1 bảng cha(bảng 1) nếu dữ liệu đó đang được sử dụng
    -- ở bảng con --> báo lỗi
);
...
ADD CONSTRAINT FK_GALLERY_ITEM FOREIGN KEY (C13_ITEM_ID) REFERENCES T01_ITEM(C01_ITEM_ID);
