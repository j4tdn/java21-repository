...
CREATE TABLE T01_ITEM(
	C01_ITEM_ID INT,
    C01_ITEM_NAME VARCHAR(255),
	C01_BUY_PRICE FLOAT NOT NULL,
	C01_ITEM_GROUP_ID INT NOT NULL,
    -- không cần khai báo constraint name cho khóa chính
    -- vì 1 table chỉ có duy nhất 1 khóa chính và nó luôn lấy name = PRIMARY_KEY
    -- chỉ dùng cách này khi khảng có khóa chính (chứa nhiều hơn 1 COLUMN)
...
-- BIT(1) --> chỉ nhận giá trị 0,1 --> 1 giá trị 
-- BIT(5) --> chỉ nhận giá trị 0,1 --> tối đa 5 giá trị
-- TINYINT(1) --> chỉ nhận --> 9   --> tối đa 1 ký tự, muốn 0, 1 thì phải thêm CHECK
...
-- PRIMARY KEY yêu cầu KDL TEXT/BLOB không được vì chưa chỉ định LENGTH cho column
...
-- ENABLE FOREIGN KEY CHECK TABLE
