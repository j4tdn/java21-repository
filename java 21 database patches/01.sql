...
CREATE TABLE java21_shopping.T02_ITEM_GROUP(
...
CREATE TABLE T01_ITEM(
    ...
    -- không cần khai báo constraint name cho khóa chính 
    ...
    -- khi xóa dòng dữ liệu ở bảng cha(bảng 1), nếu dữ liệu đó đang được sử dụng ở bảng con --> báo lỗi
    CONSTRAINT FK_ITEM_ITEMGROUP FOREIGN KEY (C01_ITEM_GROUP_ID) REFERENCES T02_ITEM_GROUP(C02_ITEM_GROUP_ID),
...
CREATE TABLE T06_SIZE(
    ...
    -- TINYINT(1) --> chỉ nhận 0 -> 9 --> tối đa 1 ký tự, muốn 0,1 thì phải thêm CHECK
	C06_GENDER BIT(1) NOT NULL,
	C06_SIZE_DESC TEXT NOT NULL,
    CONSTRAINT CHECK_SIZE_NAME CHECK (C06_SIZE_NAME IN ('S', 'M', 'XL', 'XXL'))	
);
...
ALTER TABLE T13_GALLERY ADD CONSTRAINT FK_GALLERY_ITEM FOREIGN KEY (C13_ITEM_ID) REFERENCES C01_ITEM_ID(C01_ITEM_ID);
