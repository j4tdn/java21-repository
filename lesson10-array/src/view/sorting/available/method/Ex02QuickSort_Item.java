...
public class Ex02QuickSort_Item {
	public static void main(String[] args) {
		...
		// sort array of object[Integer, Double, Item...]
		/*
		 Cách 1: Arrays.sort(Object[] a);
		 + Lúc compile a là mảng kiểu Obj 
		 	--> lúc runtime có thẻ nhận vào bất kỳ KDL nào
		 + Để biết được mảng a sắp xếp tăng dần, giảm dần theo thuộc tính nào
		   ...
		   Lúc runtime, kiểm tra KDL của mảng a truyền vào[Obj] có phải là
		   KDL con của interface Comparable<?> hay không
		   + Nếu class(KDL) truyền vào là con của Comparable<?> 
		     -- compile: hàm sort được gọi tuwfngf compareTo của Comparable<?>
		...
		/*
		 Cách 2: Arrays.sort(T[] a, Comparator<? super T> c);
			...
			T nếu nó có implements Comparable<?> vẫn ửu tiên Comparator
		 */
		...
		generate("3. Sort items by storeId[asc], created[desc]", items);
	}
}
