...
public class HibernateItemGroupDao extends GenericDao implements ItemGroupDao {
	...
	*/
	...
	public List<ItemGroupDto> getItemGroupDetails() {
		...
		/*
		 Trong Hibernate 6, setResultTransformer được thay thế bằng setTupleTransformer.
		 Do đó, bạn có thể sử dụng setTupleTransformer(Transformers.aliasToBean(ItemGroupDto.class)) như trên để thay thế setResultTransformer.  
		 * */
	}
...
//	}
	...
	private static final String Q_COUNT_ITEMS_BY_ITEM_GROUP = ""
		    + "SELECT itg.C02_ITEM_GROUP_ID, \n"
		    + "       itg.C02_ITEM_GROUP_NAME,\n"
		    + "       sum(itd.C12_AMOUNT) AS TOTAL_OF_ITEMS, \n"
		    + "       group_concat(concat(it.C01_ITEM_ID, '-', it.C01_ITEM_NAME, '-', itd.C12_SIZE_ID, '-', itd.C12_AMOUNT)) AS itemDetails \n"
	...
	public List<ItemGroupDto> countItemsByItemGroup() {
		...
		for(Object[] row : rows) {
			...
			dto.setAmountOfItems(Integer.parseInt(String.valueOf(row[2])));
			dto.setItemDetails(String.valueOf(row[3]));
