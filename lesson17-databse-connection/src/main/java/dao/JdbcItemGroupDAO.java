...
public class JdbcItemGroupDAO extends GenericDao implements ItemGroupDAO {
	...
	private static final String GET_ITEM_GROUP_DETAILS = ""
			...
			+ "       sum(t12.C12_AMOUNT) " + ItemGroupDto.PROP_AMOUT_OF_ITEMS + ",\n"
	...
	private static final String TS_UPDATE_Q2 = ""
	...
	public void updateNGroups() {
		...
		try {
			...
			pst.setString(1, "Item Group 10 - updated");
			pst.setInt(2, 10);
			pst.executeUpdate();
			
			// sql2
			pst = connection.prepareStatement(TS_UPDATE_Q2);
			pst.setInt(1, 18);
			pst.executeUpdate();
	...
	public List<ItemGroup> getAll() {
		return getElements(GET_ALL_ITEM_GROUP, ItemGroupTransformer::transformItemGroup);
	}
	...
	public void save(List<ItemGroup> groups) {
		try {
			...
			// if size list of is not divided by BATCH_SIZE
	...
	public void mergeP(ItemGroup group) {
		try {
			...
			// pst.setInt(1, group.getId() != null ? group.getId() : 0);
