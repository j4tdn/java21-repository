...
CREATE TABLE t04_project (
    ...
    CONSTRAINT FK_MANAGER_T04 
		FOREIGN KEY (C04_MANAGER_ID) 
			REFERENCES t02_employee(C02_ID)
);

CREATE TABLE t05_employees_in_project (
    C05_PROJECT_ID INT NOT NULL,
    C05_MANAGER_ID INT NOT NULL,
    C05_WORKING_TIME INT,
    PRIMARY KEY(C05_PROJECT_ID, C05_MANAGER_ID),
    CONSTRAINT FK_DEPARTMENT_T05 
		FOREIGN KEY (C05_PROJECT_ID) 
			REFERENCES t04_project(C04_ID),
    CONSTRAINT FK_MANAGER_T05 
...
FROM t02_employee
WHERE DATEDIFF(CURRENT_DATE(), C02_COMPANY_JOIN_DATE) >= 2 * 365; 
