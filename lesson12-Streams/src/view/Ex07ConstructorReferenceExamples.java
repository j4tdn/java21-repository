...
public class Ex07ConstructorReferenceExamples {
	...
	public static void main(String[] args) {
		...
		// có hàm nào, có tham số là Supplier<T>, cần truyền supplier cho bất kỳ đối tượng nào
		...
		// Dạng 3: Constructor reference cho hàm khởi tạo 1 tham số 
	
		Function<BigDecimal, TestCon> f = (a4) -> new TestCon(a4);
		System.out.println("F1 --> " + f.apply(BigDecimal.valueOf(20)));
		
		f = TestCon::new;
		System.out.println("f2 --> " + f.apply(BigDecimal.valueOf(21)));
		...
		System.out.println("ftc -> " +ftc.apply(new TestCon(1, "TC11", LocalDate.now(), BigDecimal.TEN)));
	...
	static class TestCon {
		...
		public TestCon(String line) {
			...
			if(tokens.length == 4) {
				...
				this.a2 = tokens[1];
				this.a3 =  LocalDate.parse(tokens[2], DateTimeFormatter.ofPattern("dd.mm.yyy"));
		...
		public static TestCon transfer(String line) {
			...
			if(tokens.length == 4) {
				...
				String a2 = tokens[1];
				LocalDate a3 =  LocalDate.parse(tokens[2], DateTimeFormatter.ofPattern("dd.mm.yyy"));
	...
	private static TestCon test(Integer integer1, String string2, LocalDate localdate3, BigDecimal bigdecimal4) {
		return null;
	}
}