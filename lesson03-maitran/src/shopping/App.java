package shopping;

import java.time.LocalDateTime;

public class App {

	public static void main(String[] args) {
		/*
		   cl: KHI, Adam, 123, NewYork
		   c2: KH2, Kate, 243, California  
		   i1: SSIO+, Android, Black, 620, 
		   i2: SS20U, Android, Green, 840
		   i3: IP4, IOS, White, 280
		   i4: IP12, IOS, Black, 880
		   i5: WP8, WindowPhone, Blue, 560
		 */
		
		Customer c1 = new Customer("KHI", "Adam", "123", "NewYork");
		Customer c2 = new Customer("KH2","Kate","243","California");
		
		Item i1 = new Item("SSIO+", "Android", "Black", 620);
		Item i2 = new Item("SS20U", "Android", "Green", 840);
		Item i3 = new Item("IP4", "IOS", "White", 280);
		Item i4 = new Item("IP12", "IOS", "Black", 880);
		Item i5 = new Item("WP8", "WindowPhone", "Blue", 560);
		
//		ItemDetail id1 = new ItemDetail(i1, 3)
		
		Order o1 = new Order(c1, new ItemDetail[] {
				new ItemDetail(i1, 3),
				new ItemDetail(i4, 5),
				new ItemDetail(i2, 1)
		}, LocalDateTime.of(2023, 9, 14, 10, 10, 10));
		
		Order o2 = new Order(c2, new ItemDetail[] {
				new ItemDetail(i3, 2),
				new ItemDetail(i5, 1)
		}, LocalDateTime.of(2023, 5, 7, 20, 10, 12));
		
		Order o3 = new Order(c2, new ItemDetail[] {
				new ItemDetail(i4, 1)
		}, LocalDateTime.of(2023, 5, 8, 9, 28, 32));
		
		
		
		System.out.println("Oder 1: " + OrderUtils.export(o1));
		System.out.println("Oder 2: " + OrderUtils.export(o2));
		System.out.println("Oder 4: " + OrderUtils.export(o3));
		// 2 cách
		// Cách 1: Write method export in Order class: no-parameter
		// Cách 2: Write method export in another class: export(order)
	}
}
