...
-- SELECT 1/nhiều table tròn db					--> SELECT * FROM t02_item_group;
-- SELECT dữ liệu ngẫu nhiên 					--> SELECT 'TEO', '12TK5';
-- SELECT ngẫu nhiên, biến, hàm của database	--> SELECT current_date();
-- Từ khóa UNION dùng để hợp nhiều dòng dữ liệu trả về

-- CTE: comon table expression --> bảng tạm
...
SELECT * FROM cte_payment_method;

INSERT INTO t05_order_status(C05_ORDER_STATUS_ID, C05_ORDER_STATUS_DESC)
...
INSERT INTO t06_size(C06_SIZE_ID, C06_SIZE_NAME, C06_GENDER, C06_SIZE_DESC)
VALUES  (1,	'S', 	0,	'Size S cho Nữ'),
		(2,	'M', 	0,	'Size M cho Nữ'),
		(3,	'L', 	0,	'Size L cho Nữ'),
...
INSERT INTO t08_title(C08_TITLE_ID, C08_TITLE_NAME)
VALUES  (1,	'Nhân viên'),
		(2,	'Trưởng bộ phận'),
		(3,	'Giám đốc'),
		(4,	'Chủ sơr hữu');

INSERT INTO t01_item(C01_ITEM_ID, C01_ITEM_NAME, C01_BUY_PRICE, C01_ITEM_GROUP_ID)
VALUES  (1,	'Áo 1', 			100, 	1),
		(2,	'Áo 2',				60,		1),
		(3,	'Áo 3',				80,		1),
		(4,	'Quần 4',			40,		2),
...
WITH CTE_ITEM_DETAIL_ODD AS (
	SELECT it.C01_ITEM_ID ITEM_ID, 
           ...
           it.C01_BUY_PRICE * 2 + s.C06_SIZE_ID * 5 AS PRICE,
           125 AS AMOUNT
	...
	SELECT it.C01_ITEM_ID ITEM_ID, 
		   s.C06_SIZE_ID SIZE_ID,
		   (SELECT elt(floor(rand() * 7 + 1), 'Đỏ', 'Cam', 'Vàng', 'Lục', 'Lam', 'Chàm', 'Tím')) COLOR,
           it.C01_BUY_PRICE * 2 + s.C06_SIZE_ID * 5 + 20 AS PRICE,
           280 AS AMOUNT
		FROM t01_item it, t06_size s
	WHERE (it.C01_ITEM_ID MOD 2 = 0) 
		AND (s.C06_SIZE_ID MOD 2 = 0)
...
-- CALL PROCEDURE insert_into_cust
TRUNCATE t11_customer;
CALL insert_into_cust(10);
